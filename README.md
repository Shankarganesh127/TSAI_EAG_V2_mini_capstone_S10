...
S10Share executes Python generated by Decision through a custom AST-based executor, including wrappers that translate generated calls into MCP calls. Momentum's normal query path does not execute arbitrary LLM-generated Python. Action returns a validated response, while memory and local RAG are called through normal application functions. MCP servers remain available for explicit integration and smoke testing.
